...
| Slow catalogue refresh | `MARKET_DATA_SLOW_REFRESH_MS`, default 60 000 ms. A sampled-demo (historical account) command that finds a slow contract's row older than its 45 s headroom re-reads it through the collector's `ticker/` route, so a faster catalogue is not required; in the single free service it would be load on the shared 0.1-CPU container. |
...
Spot ticker WS does not publish bid/ask. A **single spot bulk REST snapshot every five seconds**, through the existing ticker cache, refreshes those supplied values. There is no per-symbol orderbook or REST fan-out. Linear receives supplied bid/ask and derivatives fields in its ticker stream. Reconnect obtains a REST snapshot through the same cache before resubscribing. Cached REST data cannot overwrite a newer WS event. If a WS tick arrives while a newer REST bid/ask observation is in flight, spot bid/ask merge independently without rewinding the WS price or advancing its freshness; a per-row REST observation time rejects older bid/ask snapshots.
...
[Bybit connection limits and heartbeat](https://bybit-exchange.github.io/docs/v5/ws/connect), [ticker snapshot/delta fields](https://bybit-exchange.github.io/docs/v5/websocket/public/ticker), [instrument pagination](https://bybit-exchange.github.io/docs/v5/market/instrument), [Render private-network region boundary](https://render.com/docs/private-network). Runtime smoke results and deterministic measurements are in [QA evidence](qa/bybit-live-market-data/README.md).
