...
## Owner-approved original book and presentation-only order families

- Owner explicitly approved the pre-Bybit compact navy depth ladder and clarified that additional order families are presentation forms only.
- Futures now renders the original OrderBookPanel with current precise formatting and whole-row sizing retained. Desktop book width is 216px at a 1440px viewport (previously 228px); maximum 280px. Original navy background, dense 22px rows, full depth bars and quote-currency totals restored. Existing aggregation and exact price-pick behavior retained. Full 761-symbol catalogue and native TradingView remain.
- Shared presentation tabs/fields added to Futures and CFD: Limit, Market, Stop, Take-profit, OCO. Stop/Take-profit include trigger and Limit/Market execution choice; OCO has TP, stop trigger and stop execution fields. New field values never reach an execution API or the local paper store. Futures preserves existing Limit/Market execution; CFD preserves only its existing local Market path. Unsupported form submit buttons and Enter are guarded, with a short availability tooltip. No new financial engine, backend support claim or hidden synthetic order.
- TypeScript/production build PASS. Futures order-panel suite: 52 PASS, including three new conditional-family no-write cases and existing whitelist guards. Spot book suite retains its previously recorded formatting assertion failure (no change to its component in this follow-up).
- Browser: Futures and CFD OCO fields/disabled submission verified at 1440 and 390; all five tab labels fit mobile, no page horizontal overflow. Futures margin/leverage bottom 164px precedes tabs top 174px (no overlap). Native TradingView successfully rendered in this run. Screenshots: futures-original-book-oco.png, cfd-oco-presentation.png, futures-oco-mobile.png, cfd-oco-mobile.png.
