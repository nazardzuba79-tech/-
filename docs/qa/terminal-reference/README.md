...
Final owner-approved release: column-only Price/24h sorting cycles descending, ascending and default (eligible BTC first, then real volume). Scrollbar is 3px on Chromium with native thin fallback. Final validation: 277 passing, the same eight baseline failures, zero new failures; TypeScript/build pass. See release-tests.json, release-sort.json, release-1440.jpg and release-390.jpg. Current native TradingView preserved by owner choice. Production publication explicitly requested after these refinements.
