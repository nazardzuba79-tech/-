...
## 2026-09-09 — Codex — $300 deposit minimum and USDT TRC20 audit

- Agent: Codex. Owner-requested branch `codex/deposit-minimum-300-trc20-audit` from freshly fetched `origin/main` `e51dccbc10a2ee6fe2189d9cc8b2c1daa02e0426`; clean start. Implementation commit: `12e55e42ffd2d026bc1367b6863bc857ae7a51f9`. No merge or deployment.
- Material files: `src/config/limits.ts`, `src/config/chains.ts`, `src/services/DepositService.ts`, deposit API route, Tron verifier/address normalization, both deposit modals, shared deposit config/hook/API helper, seven-language support copy, focused tests, `scripts/qa-deposit-minimum.cjs`, and `docs/DEPOSIT_MINIMUM_300_TRC20_AUDIT.md`.
- Backend owns $300 and USD peg policy; both forms consume the opt-in config envelope. Original unparameterized chains API preserved. Existing market APIs provide volatile equivalents; unavailable prices show USD only, and the backend's unavailable-price safety rule stays intact.
- Additional credit-path fixes: normalize Tron hex/Base58Check event addresses, reject invalid event data, canonicalize asset symbols for stablecoin valuation, and canonicalize Tron hash/chain case for duplicate protection. Treasury configuration/addresses, referral 5% and math, transaction rollback, other trading/withdrawal/copy logic and prior agents' presentation work preserved. Support copy points to the form instead of duplicating the threshold.
- Read-only production audit: intended USDT contract/6 decimals configured on Render, nonempty Tron treasury override exists in Neon, no historical Tron deposit exists. An unrelated public TronGrid transfer passed the actual verifier. **TRC20 CODE/CONFIG VERIFIED / LIVE END-TO-END DEPOSIT NOT YET PROVEN.** No production financial writes or private address export.
- Validation: backend/frontend TypeScript PASS; production Vite build PASS using in-process esbuild-wasm for restricted Windows native child pipes. Final focused tests 126/126; full Jest 1998/2001, with the same three baseline hash failures (avatarIdentityPresentation, nazaraCardPresentation, walletUxRefinement), versus measured base 1918/1921. No new failures. Final support-copy change followed by frontend TS/build and focused rerun; no executable behavior changed after full Jest.
- Browser QA: actual production bundle and deposit GET routes with isolated nonpayable treasury/market fixtures; both forms at 1440x1000 and 390x900, USDT TRC20 $300, BTC equivalent and unavailable-price fallback. Twelve captures, no console errors/warnings or horizontal overflow. Local artifacts are in the task's `outputs/deposit-minimum-300` folder.
- Unresolved / next: owner review, then separately authorized production end-to-end verification. Existing PENDING/BELOW_MINIMUM rows remain idempotently recorded and are not retroactively reprocessed by this patch. No claim of live exchange deposit proof.
