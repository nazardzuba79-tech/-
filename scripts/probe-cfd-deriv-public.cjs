 ...
 * credentials or VOLTEX financial state. It validates symbol discovery and
 * asks for one latest history point per mapped symbol; it does NOT grant
 * commercial/execution rights. */
...
try{
 ...
 socket.on('message',data=>{
  ...
  if(raw?.msg_type==='active_symbols'){
    ...
    if(d.mapped.length!==13||d.missing.length||d.ambiguous.length){report.errors.push('coverage_not_13');return finish(1);}
