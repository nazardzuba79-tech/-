...
const SCALE = Array.from({ length: 515 }, (_, i) => [`SC${i}/USDT`, String(1 + i / 100), '0.50', '2', '1', '1000', String(5_000_000 - i * 1000)]);
// 520 listed contracts, of which only the first five carry a reference
// ticker: the unpriced tail is the case the old 40-market cap hid.
const FUTURES_SYMBOLS = [...PAIRS.map(([p]) => p), ...SCALE.map(([p]) => p)];
...
server.listen(PORT, '127.0.0.1', () => {
  console.log(`[qa-market-universe-scale] read-only fixture server on http://127.0.0.1:${PORT}`);
});
