...
(async()=>{
 ...
 for(const width of [1920,1280,768,390]){
  ...
  await page.locator('.cfd-owned-chart-canvas canvas').first().waitFor({state:'visible',timeout:25000});await page.waitForFunction(()=>document.querySelector('.cfd-owned-chart')?.getAttribute('data-chart-status')==='ready',null,{timeout:25000});
  // Current mobile terminal intentionally starts on Chart and hides the order form.
  // The QA must navigate the same public mobile tabs a trader uses instead of
  // assuming every desktop panel is simultaneously visible below 900px.
  if(width<=900){await page.locator('#mobile-trade-trade').click();await page.waitForFunction(()=>document.querySelector('.terminal')?.getAttribute('data-mobile-tab')==='trade');}
  await page.locator('.cfd-order-panel form').waitFor({state:'visible',timeout:10000});
  const price=await page.locator('.cfd-option.active .cfd-price').innerText().catch(()=>selectedSymbol);const input=page.locator('.cfd-order-panel input[type=number]');await input.fill('0.01');const submit=page.locator('.cfd-order-panel button[type=submit]');await submit.waitFor({state:'visible'});if(await submit.isDisabled())throw new Error(`Order remained disabled at ${width}px`);await submit.click();
  if(width<=900){await page.locator('#mobile-trade-account').click();await page.waitForFunction(()=>document.querySelector('.terminal')?.getAttribute('data-mobile-tab')==='account');}
