'use strict';
/** Disposable end-to-end browser QA for the working CFD practice terminal.
 * Real public quotes/OHLC are fetched server-side. The browser is loopback
 * only and every non-GET request is rejected, proving practice orders do not
...
const noDb=new Proxy({},{get(){throw new Error('Database access forbidden in CFD practice QA');}});
...
(async()=>{
 ...
 for(const width of [1920,1280,768,390]){
  const context=await browser.newContext({viewport:{width,height:900},serviceWorkers:'block'});await context.addInitScript(()=>{localStorage.setItem('exchange_lang','ru');localStorage.setItem('exchange_token','local-cfd-practice-qa');localStorage.removeItem('voltex_cfd_practice_v1');});
  ...
  await page.locator('.cfd-owned-chart-canvas canvas').first().waitFor({state:'visible',timeout:25000});await page.locator('.cfd-order-panel form').waitFor({state:'visible',timeout:10000});
  const price=await page.locator('.cfd-option.active .cfd-price').innerText();const input=page.locator('.cfd-order-panel input[type=number]');await input.fill('0.01');const submit=page.locator('.cfd-order-panel button[type=submit]');await submit.waitFor({state:'visible'});if(await submit.isDisabled())throw new Error(`Practice order remained disabled at ${width}px`);await submit.click();
  await page.locator('.cfd-position-content tbody tr').first().waitFor({state:'visible',timeout:5000});const openText=await page.locator('.cfd-position-content').innerText();if(!openText.includes('XAUUSD'))throw new Error(`Practice position missing at ${width}px`);
  const close=page.locator('.cfd-closeBtn').first();await page.waitForFunction(()=>{const b=document.querySelector('.cfd-closeBtn');return b&&!b.disabled;},{timeout:10000});await close.click();await page.locator('.cfd-tab').nth(1).click();await page.locator('.cfd-position-content tbody tr').first().waitFor({state:'visible',timeout:5000});const historyText=await page.locator('.cfd-position-content').innerText();if(!historyText.includes('XAUUSD'))throw new Error(`Practice history missing at ${width}px`);
  const result=await page.evaluate(()=>({overflow:document.documentElement.scrollWidth>innerWidth+1,rows:document.querySelectorAll('.cfd-option').length,forms:document.querySelectorAll('.cfd-terminal form').length,submits:document.querySelectorAll('.cfd-terminal button[type=submit]').length,canvases:document.querySelectorAll('.cfd-owned-chart-canvas canvas').length,practice:document.querySelector('.cfd-practice-badge')?.textContent||null}));report.scenarios.push({width,price,...result});if(result.overflow)report.findings.push(`Horizontal overflow ${width}px`);if(result.forms!==1||result.submits!==1)report.findings.push(`Practice order ticket incomplete ${width}px`);if(result.canvases<1)report.findings.push(`Owned chart missing ${width}px`);await page.screenshot({path:path.join(OUT,`cfd-working-${width}.png`),fullPage:true});await context.close();activePage=null;
 }
 // Homepage uses the same routed prices: verify GOLD + exact WTI once.
