...
async function run(){
  ...
  try{
    ...
    const market=await (await worker.fetch(new Request("https://market.voltextech.net/market/display?case=home"))).json();
    ...
    const cfd=await (await worker.fetch(new Request("https://market.voltextech.net/cfd/display/tickers?case=cfd"))).json();
    ...
    const cfdBars=await (await worker.fetch(new Request("https://market.voltextech.net/cfd/display/candles/XAUUSD?interval=1h&limit=50&case=cfd"))).json();
