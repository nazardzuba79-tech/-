...
export async function getSpotPublicCandles(pair: string, interval: string, limit: number, signal?: AbortSignal, endTime?: number): Promise<{ candles: Candle[] }> {
  ...
  if (endTime !== undefined) {
    const response = await fetchJson(`${APP_API_BASE}/market/external/candles/${pairSlug(pair)}?interval=${encodeURIComponent(interval)}&limit=${Math.min(1000,limit)}`, signal);
  ...
  if (!productionSite()) {
    const response = await fetchJson(`${APP_API_BASE}/market/external/candles/${pairSlug(pair)}?interval=${encodeURIComponent(interval)}&limit=${Math.min(720,limit)}`, signal);
    return { candles: parseEdgeSpotCandles(response, pair, interval) };
  }
