...
export const bankingErrorText=(error:unknown)=>{const code=error instanceof BankingApiError?error.code:'';return ({below_minimum:'Сума нижча за мінімальний еквівалент $2,500.',insufficient_available_balance:'Недостатньо доступного балансу в обраному активі.',asset_price_unavailable:'Поточна USD-ціна недоступна. Розміщення тимчасово вимкнено.',program_unavailable:'Програма недоступна.',idempotency_key_reused:'Повторна команда не збігається з початковою.',auth_required:'Увійдіть в акаунт.'} as Record<string,string>)[code]||'Не вдалося виконати операцію.'};
