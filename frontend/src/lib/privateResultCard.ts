...
/** How far the 1440-tall raster slides up inside the 1215-tall card. */
const ARTWORK_LIFT = 225;
...
function warmArtwork(): string {
  ...
  // THE 225px COME OFF THE TOP, NOT THE BOTTOM.
  // The supplied 1080x1440 raster is empty dark sky down to about y=500: the
  // arrow tip starts there and the ribbon's base runs to the very bottom
  // edge. Shortening the card to 1215 by cropping the bottom would slice
  // through the ribbon; sliding the raster up by ARTWORK_LIFT takes the crop
  // out of the empty sky instead, so the artwork is neither cropped through
  // anything it draws nor distorted — same image, same scale, same aspect.
  //
  // The bags are drawn in card coordinates on top of the raster and are
  // glued to the ribbon, so they move up by exactly the same amount. Their
  // SCALE is untouched: they keep the ~17% enlargement at the same pixel
  // size, which against a shorter card reads slightly larger, not smaller.
  return `<g data-artwork="warm-rise" aria-hidden="true"><image href="${privateCardArtwork}" x="0" y="${-ARTWORK_LIFT}" width="1080" height="1440"/><g data-artwork="money-bags">${bag(872,876-ARTWORK_LIFT,1.055)}${bag(627,1133-ARTWORK_LIFT,.866)}</g></g>`;
}
...
export function privateResultCardSvg(card: PrivateResultCard): string {
  ...
  // replay/live distinction stays in the server snapshot and in `card.mode`,
  // which still gates rendering below, but it is never drawn. Nothing on the
  // card says Test, Demo, Simulation, Preview or fixture.
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1215" viewBox="0 0 1080 1215" role="img" aria-labelledby="card-title" font-family="Arial,Helvetica,sans-serif">
...
<rect width="1080" height="1215" fill="url(#bg)"/>${warmArtwork()}<rect width="1080" height="1215" fill="url(#shade)"/>${brand()}
<g font-variant-numeric="tabular-nums lining-nums"><g transform="translate(80 248)">
...
<text data-field="roi-label" x="0" y="136" font-size="23" fill="#c1b9b0">ROI</text>
<text data-field="roi-line" x="0" y="${146 + roiSize}" fill="${accent}" font-weight="700"${fit(roi + (roi !== '—' ? '%' : ''), roiSize, 884)}><tspan data-field="roi-number" font-size="${roiSize}" letter-spacing="-2.7">${escapeXml(roi)}</tspan>${roi !== '—' ? `<tspan data-field="roi-unit" dx="4" dy="0" font-size="${roiSize}" letter-spacing="-2.7">%</tspan>` : ''}</text>
<text data-field="profit-label" x="0" y="304" font-size="23" fill="#c1b9b0">Profit</text>
<text data-field="profit-line" x="0" y="${316 + profitSize}" fill="#f5f1ea" font-weight="700"><tspan data-field="profit-number" font-size="${profitSize}" letter-spacing="-.65"${fit(profit, profitSize, 754)}>${escapeXml(profit)}</tspan><tspan data-field="profit-unit" dx="22" font-size="25" fill="#d9c5a3" font-weight="400" letter-spacing=".6">USDT</tspan></text>
<path d="M0 440H400" stroke="#bda586" stroke-opacity=".22"/>
<text data-field="entry-label" x="0" y="502" font-size="23" fill="#c4bab0">Цена Входа</text>
<text data-field="entry-price" x="0" y="546" font-size="34" font-weight="700" fill="#f0e9df"${fit(entryPrice, 34, 440)}>${escapeXml(entryPrice)}</text>
