...
test('native chart owns timeframe controls; parent does not claim a mirrored interval',async()=>{
  await render();expect(config().interval).toBe('15');expect(host.textContent).not.toContain('BTC/USDT · 15m');const old=materialize();
  expect(Array.from(host.querySelectorAll('button')).some(b=>['1h','1D','15m'].includes(b.textContent??''))).toBe(false);
  await render('ETH/USDT');expect(old.isConnected).toBe(false);expect(config().symbol).toContain('ETHUSDT');
...
test('native TradingView indicators and volume controls are enabled without fake parent controls',async()=>{
  await render();expect(config().hide_top_toolbar).toBe(false);expect(config().studies).toEqual([]);expect(config().hide_volume).toBe(false);
  expect(host.querySelector('select[aria-label="Chart indicator"]')).toBeNull();expect(Array.from(host.querySelectorAll('button')).some(b=>b.textContent==='VOL')).toBe(false);expect(host.querySelectorAll('script')).toHaveLength(1);
});
...
test('verified CFD mapping and visible licensed attribution remain without a mirrored parent toolbar',async()=>{
  await render('XAUUSD','cfd');expect(config().symbol).toBe('OANDA:XAUUSD');expect(host.textContent).toContain('XAU/USD');expect(host.textContent).toContain('by TradingView');
  expect(host.querySelector('.terminal-chart-controls')).toBeNull();expect(host.querySelector('.voltex-tradingview-chart__copyright a')).not.toBeNull();
});
