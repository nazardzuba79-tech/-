...
test('all literal presentation image URLs point to present production assets with valid format signatures', () => {
  ...
  expect(paths.has('/cards/crypto-card-final/voltex-watch-wrist-original.png')).toBe(true);
...
test('the production card route retains RequireAuth and never opts out of real application actions', () => {
  ...
  // Production promotion must not pull in the isolated review application.
  // Explicit reviewOnly read/write rejection remains covered in the controller tests.
  expect(existsSync(resolve(frontend, 'src/review/main.tsx'))).toBe(false);
  expect(read('src/pages/crypto-card-final/CardApplication.tsx')).toContain('reviewOnly = false');
