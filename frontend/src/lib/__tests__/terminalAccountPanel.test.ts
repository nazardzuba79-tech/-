...
test('real terminal pages preserve tab navigation and mounted readers while the body is compact', () => {
  ...
  // Public/Real futures keeps the same readers; only the owner's native Demo view (which never shows
  // the real account panel) skips polling the real account.
  expect(futures).toContain('useFuturesAccount(nativeRequested?{}:{ orders: 5000, positions: 4000 })');
