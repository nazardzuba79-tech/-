...
import * as priceChange from '../priceChange';
...
/** Isolated real TSX hooks/callbacks. API fixtures never leave this harness. */
function mount(file: string, options: any = {}) {
  ...
  new Function('require', 'exports', 'window', compiled)((name: string) => {
    ...
    if (name.endsWith('/priceChange')) return priceChange;
...
('TradePage %s renders only the real CFD three-column terminal', query => {
  ...
  for (const component of ['CfdChart', 'CfdOrderForm', 'CfdInstrumentList', 'CfdTickerBar']) expect(all.find(n => n.type === page.components[component]).props.symbol).toBe(symbol);
...
test('instrument loading, unconfigured and error/retry remain distinct; no fabricated instrument rows', () => {
  ...
  expect(nodes(list.render(props)).filter(n => n.type === 'button')).toHaveLength(0);
...
test.each([
  ['tickers missing entirely', {}], ['tickers null', { configured: true, tickers: null }],
  ['tickers an object', { configured: true, tickers: { XAUUSD: '2400' } }], ['tickers a string', { configured: true, tickers: 'XAUUSD' }],
  ['whole body null', null], ['whole body a string', 'service unavailable'],
  ...
  expect(Array.isArray(state.tickers)).toBe(true);expect(state.tickers).toEqual([]);expect(state.loadError).toBe(true);expect(state.configured).toBe(true);
  expect(() => presentation.resolveCfdSymbol('XAUUSD', state.tickers)).not.toThrow();expect(presentation.resolveCfdSymbol('XAUUSD', state.tickers)).toBe('XAUUSD');
});
test('a malformed refresh keeps the last good instrument rows on screen', async () => {
  const getCfdTickers = jest.fn().mockResolvedValueOnce({ configured: true, tickers: rows }).mockResolvedValue({ configured: true, tickers: undefined });
  const hook = mount('lib/useCfdTickers.ts', { api: { getCfdTickers } });
  hook.render(); await tick();expect(hook.render().tickers.map((t: any) => t.symbol)).toEqual(rows.map(r => r.symbol));
  hook.render().reload(); await tick();const state = hook.render();
  expect(state.tickers.map((t: any) => t.symbol)).toEqual(rows.map(r => r.symbol));expect(state.tickers[0].price).toBe('2400.125');expect(state.loadError).toBe(true);
});
test('a well-formed empty list is truth, not an error', async () => {
  const state = await feedFor({ configured: false, tickers: [] });expect(state.tickers).toEqual([]);expect(state.loadError).toBe(false);expect(state.configured).toBe(false);
});
test('unusable rows are dropped and a listed unavailable price stays null', async () => {
  const state = await feedFor({configured: true,tickers: [
    { symbol: 'XAUUSD', name: 'Gold', price: '2400.125', changePercent24h: '0.23' },
    { symbol: '', name: 'No symbol', price: '1.1' }, { symbol: 'EURUSD', name: 'Euro', price: null },
    { symbol: 'GBPUSD', name: 'Pound', price: 'not-a-number' }, null]});
  expect(state.tickers).toHaveLength(2);expect(state.tickers[1]).toMatchObject({symbol:'EURUSD',price:null,executionAllowed:false,status:'unavailable'});
  expect(state.tickers[0]).toMatchObject({ symbol: 'XAUUSD', name: 'Gold', price: '2400.125', changePercent24h: '0.23' });
  expect(JSON.stringify(state.tickers)).not.toContain('"price":"0"');expect(state.loadError).toBe(false);
});
test('a real zero and a numeric price survive; an unknown 24h change stays absent', async () => {
  const state = await feedFor({configured: true,tickers: [
    { symbol: 'XAUUSD', name: 'Gold', price: '0', changePercent24h: '0' }, { symbol: 'EURUSD', name: 'Euro', price: 1.12345, changePercent24h: null }]});
  expect(state.tickers[0]).toMatchObject({ symbol: 'XAUUSD', name: 'Gold', price: '0', changePercent24h: '0' });
  expect(state.tickers[1]).toMatchObject({ symbol: 'EURUSD', name: 'Euro', price: '1.12345' });expect('changePercent24h' in state.tickers[1]).toBe(false);
});
test('a row without a name falls back to its symbol rather than an empty label', async () => {
  const state = await feedFor({ configured: true, tickers: [{ symbol: 'USDJPY', price: '155.25' }] });expect(state.tickers[0]).toMatchObject({ symbol: 'USDJPY', name: 'USDJPY', price: '155.25' });
});
test('MARKET/ISOLATED form uses original sizing, leverage, USDT margin and exact open payload', async () => {
  ...
  expect(text(tree)).toContain('trade.market'); expect(text(tree)).toContain('futures.isolated');expect(text(tree)).not.toMatch(/trade.limit|futures.cross|reduceOnly/);
  ...
  tree = form.render(props);const percents = nodes(tree).filter(n => n.props?.className?.startsWith('cfd-percentBtn'));
  expect(percents.map(n => text(n))).toEqual(['0%', '25%', '50%', '75%', '100%']);percents[2].props.onClick(); tree = form.render(props);
...
const openPosition = {
  id: 'cfd-1', symbol: 'EURUSD', side: 'LONG', leverage: 10, size: '1', entryPrice: '1.1', markPrice: '1.2', liquidationPrice: '1',
  unrealizedPnl: '.1', roe: '1', realizedPnl: '0', status: 'OPEN', initialMargin: '10', openedAt: '2026-09-01T00:00:00.000Z', closedAt: null,
};
async function panelFor(positions: any, history: any = []) {
  const getCfdPositions = jest.fn().mockResolvedValue(positions);const getCfdPositionHistory = jest.fn().mockResolvedValue(history);
  const panel = mount('components/CfdPositionsPanel.tsx', { api: { getCfdPositions, getCfdPositionHistory } });
  panel.render({ refreshKey: 0 }); await tick();return { panel, tree: () => panel.render({ refreshKey: 0 }), getCfdPositions };
}
const renderedIds = (tree: any) => nodes(tree).filter(n => n.type === 'tr' && n.key).map(n => n.key);
test.each([
  ['not an array (object)', {}], ['not an array (string)', 'service unavailable'], ['missing entirely', undefined], ['null', null],
  ['array holding null', [null]], ['array holding a primitive', [42]], ['row missing its id', [{ ...openPosition, id: undefined }]],
  ['row with an empty id', [{ ...openPosition, id: '   ' }]], ['row with an unrecognised side', [{ ...openPosition, side: 'FLAT' }]],
  ['row with a non-numeric leverage', [{ ...openPosition, leverage: 'ten' }]], ['row missing its entry price', [{ ...openPosition, entryPrice: undefined }]],
])('malformed positions payload (%s) never crashes and never reads as an empty account', async (_label, payload) => {
  const { tree } = await panelFor(payload);expect(() => tree()).not.toThrow();expect(text(tree())).toContain('futures.loadPositionsError');
  expect(text(tree())).not.toContain('futures.noPositions');expect(renderedIds(tree())).toHaveLength(0);expect(text(tree())).not.toMatch(/0\.00/);
});
test('a malformed refresh keeps the last good positions on screen', async () => {
  const getCfdPositions = jest.fn().mockResolvedValueOnce([openPosition]).mockResolvedValue({ error: 'gateway' });
  const panel = mount('components/CfdPositionsPanel.tsx', { api: { getCfdPositions, getCfdPositionHistory: async () => [] } });
  panel.render({ refreshKey: 0 }); await tick();expect(renderedIds(panel.render({ refreshKey: 0 }))).toEqual(['cfd-1']);
  jest.advanceTimersByTime(4000); await tick();const tree = panel.render({ refreshKey: 0 });
  expect(renderedIds(tree)).toEqual(['cfd-1']);expect(text(tree)).toContain('futures.loadPositionsError');expect(text(tree)).not.toContain('futures.noPositions');
});
test('a legitimate empty list is still a legitimate empty list', async () => {
  const { tree } = await panelFor([]);expect(text(tree())).toContain('futures.noPositions');expect(text(tree())).not.toContain('futures.loadPositionsError');expect(renderedIds(tree())).toHaveLength(0);
});
test('a valid payload renders unchanged, real zeros included', async () => {
  ...
  const { tree } = await panelFor([openPosition, zeroed]);expect(renderedIds(tree())).toEqual(['cfd-1', 'cfd-zero']);expect(text(tree())).not.toContain('futures.loadPositionsError');
  const cells = nodes(tree()).filter(n => n.props?.className?.includes('mono')).map(n => text(n));expect(cells).toContain('0.00');expect(cells).toContain('0.00%');
});
test('absent optional figures render as a dash, never as NaN or zero', async () => {
  ...
  expect(renderedIds(tree())).toEqual(['cfd-1']);expect(text(tree())).toContain('—');expect(text(tree())).not.toContain('NaN');
});
test.each([
  ['not an array', { rows: [] }], ['array holding null', [null]], ['row with an unparseable realized PnL', [{ ...openPosition, realizedPnl: 'oops' }]],
  ...
  const { panel } = await panelFor([], payload);nodes(panel.render({ refreshKey: 0 })).find((n: any) => n.props?.id === 'cfd-tab-history').props.onClick();
  panel.render({ refreshKey: 0 }); await tick();const tree = panel.render({ refreshKey: 0 });expect(() => panel.render({ refreshKey: 0 })).not.toThrow();
  expect(text(tree)).toContain('futures.loadPositionsError');expect(text(tree)).not.toContain('futures.noPositionHistory');
});
test('a valid history payload still renders, with a real zero realized PnL', async () => {
  const closed = { ...openPosition, id: 'cfd-closed', realizedPnl: '0', status: 'CLOSED' };const { panel } = await panelFor([], [closed]);
  nodes(panel.render({ refreshKey: 0 })).find((n: any) => n.props?.id === 'cfd-tab-history').props.onClick();panel.render({ refreshKey: 0 }); await tick();
  const tree = panel.render({ refreshKey: 0 });expect(renderedIds(tree)).toEqual(['cfd-closed']);expect(text(tree)).toContain('0.00');expect(text(tree)).not.toContain('futures.loadPositionsError');
});
test('positions/history preserve polling, close API, closing state, error and liquidation status', async () => {
  let finish: any;const closeCfdPosition = jest.fn().mockImplementation(() => new Promise(resolve => { finish = resolve; }));
  ...
  panel.render({ refreshKey: 0 }); await tick(); let tree = panel.render({ refreshKey: 0 });nodes(tree).find(n => n.props?.className === 'cfd-closeBtn').props.onClick();
  expect(closeCfdPosition).toHaveBeenCalledWith('local-test-position');expect(nodes(panel.render({ refreshKey: 0 })).find(n => n.props?.className === 'cfd-closeBtn').props.disabled).toBe(true);
  finish({}); await tick(); panel.render({ refreshKey: 0 }); await tick();jest.advanceTimersByTime(4000); expect(getCfdPositions.mock.calls.length).toBeGreaterThanOrEqual(3);
  tree = panel.render({ refreshKey: 0 }); nodes(tree).find(n => n.props?.id === 'cfd-tab-history').props.onClick();panel.render({ refreshKey: 0 }); await tick(); tree = panel.render({ refreshKey: 0 });
  expect(getCfdPositionHistory).toHaveBeenCalledTimes(1); expect(text(tree)).toContain('LIQUIDATED');nodes(tree).find(n => n.props?.id === 'cfd-tab-open').props.onClick(); panel.render({ refreshKey: 0 }); await tick();
  closeCfdPosition.mockRejectedValueOnce(new Error('close rejected'));nodes(panel.render({ refreshKey: 0 })).find(n => n.props?.className === 'cfd-closeBtn').props.onClick(); await tick();expect(text(panel.render({ refreshKey: 0 }))).toContain('close rejected');
});
// Existing preservation hashes are unchanged: no order/position/Spot callback
// or financial arithmetic is modified by display-only reference integration.
test.each([
...
test('all CSS selectors are scoped and chart stays on the unchanged real TradingView mapping', () => {
  ...
  const wrapper = read('components/CfdChart.tsx');expect(wrapper).toContain('market="cfd"');expect(wrapper).toContain('trade.cfdPriceDisclaimer');
  ...
  // Current main delegates interval/indicators/volume to the native widget;
  // only instrument, locale and retry should recreate the parent embed.
  expect(chart).toContain('[symbol, locale, attempt]');
  const executable = ts.createPrinter({ removeComments: true }).printFile(ts.createSourceFile('chart.tsx', chart, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX));expect(executable).not.toMatch(/api\.|candles|orderBook|funding|openInterest/);
});
test('CFD reference-price disclaimer matches the unchanged 60-second poll in every language', () => {
  const disclaimers = readAllLocales().split('\n').filter(line => line.includes("'trade.cfdPriceDisclaimer':"));expect(disclaimers).toHaveLength(7);
  expect(disclaimers.every(line => line.includes('60') && !line.includes('30'))).toBe(true);expect(read('lib/useCfdTickers.ts')).toContain('const POLL_MS = 60_000');
});
...
test('reference metadata survives the hook and cannot inherit live permission or change',async()=>{
  const row={...rows[0],displayOnly:true,status:'live',stale:false,executionAllowed:true,referenceKind:'daily_reference',referenceLabel:'Daily reference · ECB · 2026-09-11',observationDate:'2026-09-11'};
  const state=await feedFor({configured:true,tickers:[row]});expect(state.tickers[0]).toMatchObject({displayOnly:true,executionAllowed:false,referenceLabel:row.referenceLabel,observationDate:row.observationDate});expect(state.tickers[0].changePercent24h).toBeUndefined();
});
test('display-only references render genuine zero/negative but still cannot execute',()=>{
  expect(presentation.formatCfdPrice('0','WTIUSD',true)).toBe('0.00');expect(presentation.formatCfdPrice('-1.25','WTIUSD',true)).toBe('-1.25');expect(presentation.formatCfdPrice(null,'WTIUSD',true)).toBe('—');
  const q={...rows[0],displayOnly:true,status:'live',stale:false,executionAllowed:true,providerTimestamp:Date.now(),fetchedAt:Date.now(),maxQuoteAgeMs:5000};expect(presentation.canExecuteCfdQuote(q)).toBe(false);
});
test('display-only quote disables and blocks programmatic order submission',async()=>{
  const openCfdPosition=jest.fn();const form=mount('components/CfdOrderForm.tsx',{api:{openCfdPosition}});
  const props={symbol:'XAUUSD',ticker:{...rows[0],displayOnly:true,status:'live',stale:false,executionAllowed:true,providerTimestamp:Date.now(),fetchedAt:Date.now(),maxQuoteAgeMs:5000},onPlaced:jest.fn()};
  const tree=form.render(props);expect(nodes(tree).find(n=>n.props?.type==='submit').props.disabled).toBe(true);nodes(tree).find(n=>n.type==='form').props.onSubmit({preventDefault:jest.fn()});await tick();expect(openCfdPosition).not.toHaveBeenCalled();
});
test('terminal reference label and real zero are displayed without a fabricated daily change',()=>{
  const row={symbol:'WTIUSD',name:'WTI',price:'0',displayOnly:true,status:'reference_only',referenceLabel:'Daily reference · U.S. EIA · 2026-09-11',changePercent24h:'55'};
  const list=mount('components/CfdInstrumentList.tsx');const tree=list.render({symbol:'WTIUSD',tickers:[row],configured:true,loadError:false,onRetry:jest.fn(),onChange:jest.fn()});
  expect(text(tree)).toContain(row.referenceLabel);expect(text(tree)).not.toContain('55.00%');expect(nodes(tree).find(n=>n.type===list.components.PriceCell).props.format(0)).toBe('0.00');
});
