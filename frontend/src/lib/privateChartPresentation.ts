...
export function privateChartOverlays(positions:PrivatePosition[],pair:string):ChartTradeOverlay[]{
  ...
  return positions.filter(position=>position.symbol===symbol&&position.dataStatus!=='UNAVAILABLE'&&(!position.verification||position.verification==='VERIFIED'))
    .flatMap(position=>{
      ...
      return [{id:position.id,symbol,side:position.side,leverage:Number(position.leverage),entryPrice:price,quantity:Number(position.status==='OPEN'?position.quantity:position.initialQuantity??position.quantity),pnl,status:position.status,
