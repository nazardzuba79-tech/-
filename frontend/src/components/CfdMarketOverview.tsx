...
export function CfdMarketOverview({symbol,ticker}:{symbol:string;ticker?:CfdTickerRow}){
  ...
  return <aside className="cfd-market-overview" aria-label={copy.marketOverview}>
    ...
    <dl className="cfd-overview-grid"><div><dt>{copy.session}</dt><dd>{ticker?.status==='market_closed'?copy.closed:ticker?.price?copy.open:copy.unavailable}</dd></div><div><dt>{copy.updated}</dt><dd>{asOf||'—'}</dd></div></dl>
  </aside>;
}
