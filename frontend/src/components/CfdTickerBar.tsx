...
export function CfdTickerBar({symbol,ticker}:{symbol:string;ticker?:CfdTickerRow}){
  const{t}=useLanguage(),change=ticker?parseChangePercentOrNull(ticker.changePercent24h,symbol):null,state=cfdDisplayState(ticker);
  return <header className="cfd-ticker-bar">
    <div className="cfd-selected-instrument"><span className="cfd-symbol mono">{symbol}</span><span className="cfd-product-badge">MARKET</span><span className="cfd-instrument-name">{ticker?.name??'—'}</span></div>
    ...
    <div className="cfd-ticker-metric cfd-market-state"><span>Status</span><span className={`cfd-state-pill cfd-state-${state.tone}`}>{state.label}</span></div>
  </header>;
}
