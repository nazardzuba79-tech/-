...
export function HomeHeroAssets({ market, englishLabels = false }: { market: HomeMarket; englishLabels?: boolean }) {
  ...
  const cfdRows = market.cfd?.tickers ?? [];
  const gold = cfdRows.find(row => row.symbol === 'XAUUSD');
  const wti = cfdRows.find(row => row.symbol === 'WTIUSD');
  const brent = cfdRows.find(row => row.symbol === 'XBRUSD');
  const oil = finiteQuote(wti?.price) !== null ? wti : finiteQuote(brent?.price) !== null ? brent : wti ?? brent;
  const goldPrice = finiteQuote(gold?.price);
  const oilPrice = finiteQuote(oil?.price);
  const oilPair = oil?.symbol === 'XBRUSD' ? 'XBR/USD' : 'WTI/USD';
  const oilHistory = oil?.symbol ? market.cfdPriceHistory?.[oil.symbol] ?? [] : [];
  const rows = [
    { key: 'btc', title: 'BTC / USDT', price: market.hero.pair === 'BTC/USDT' ? market.hero.livePrice ?? btc?.price : btc?.price,
      change: btc?.change, Icon: Bitcoin, points: market.priceHistory['BTC/USDT'] ?? [], note: market.tickersStale ? copy.quote : 'BTC / USDT', source: null },
    { key: 'gold', title: englishLabels ? 'GOLD' : copy.gold, price: goldPrice, change: finiteQuote(gold?.changePercent24h), Icon: GoldIcon,
      points: gold ? market.cfdPriceHistory?.XAUUSD ?? [] : [], note: goldPrice !== null ? `${copy.quote} · XAU/USD` : copy.unavailable,
      source: goldPrice !== null ? `${copy.quote} · XAU/USD` : null },
    { key: 'oil', title: englishLabels ? 'OIL' : copy.oil, price: oilPrice, change: finiteQuote(oil?.changePercent24h), Icon: OilIcon,
      points: oilHistory, note: oilPrice !== null ? `${copy.quote} · ${oilPair}` : copy.unavailable,
      source: oilPrice !== null ? `${copy.quote} · ${oilPair}` : null },
  ];
