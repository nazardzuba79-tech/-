...
export function HomeHeroAssets({ market, englishLabels = false }: { market: HomeMarket; englishLabels?: boolean }) {
  ...
  const rows = [
    ...
    { key: 'oil', title: copy.oil, price: finiteQuote(oil?.price), change: finiteQuote(oil?.changePercent24h), Icon: OilIcon, points: oil ? market.cfdPriceHistory?.WTIUSD ?? [] : [], note: oil ? `${copy.quote} · WTI/USD` : copy.unavailable },
  ];
  return <div className="vx-global-assets">
    {rows.map(({ key, title, price, change, Icon, points, note }, index) => <div className={`vx-asset-pill vx-asset-${key}`} key={key}
      ...
      {(key === 'gold' && gold || key === 'oil' && oil) && <span className="vx-asset-source">{copy.quote}</span>}
