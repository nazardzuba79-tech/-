...
export function SapphireTerminal({market}:{market:HomeMarket}){
  ...
  return <div id="home-live-terminal" className="terminal-screen-content hs-terminal-detail" data-stale={stale}>
    <div className="ts-nav"><span className="ts-brand"><LogoMark size={17}/> VOLTEX</span><span className="ts-active">Spot</span><span>Markets</span><span>Futures</span><span className="feed-state"><i className="hs-receipt" key={stale?'stale':updated??'loading'}/>{stale?'Stale':'6h snapshot'}</span></div>
    ...
    <div className="ts-footer"><span>{stale?'Stale market data':'Public market snapshot'} · {market.tickerSource||'—'}</span><span>Updated {updated?time(updated):'—'} UTC</span></div>
  </div>;
}
