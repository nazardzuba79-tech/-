...
export function SapphireTerminal({market}:{market:HomeMarket}){
  ...
  return <div id="home-live-terminal" data-sampled-preview="true" className="terminal-screen-content hs-terminal-detail" data-stale={stale}>
    <div className="ts-nav"><span className="ts-brand"><LogoMark size={17}/> VOLTEX</span><span className="ts-active">Spot</span><span>Markets</span><span>Futures</span><span className="feed-state"><i className="hs-receipt" key={stale?'stale':updated??'market'}/>{stale?'Delayed':'Market data'}</span></div>
    ...
    </div><div className="hs-recent-trades"><div className="hs-trades-title">Recent market trades <small>{f.tradesStatus==='error'?'Delayed':pair}</small></div><div className="hs-trade-head"><span>Time</span><span>Pair</span><span>Side</span><span>Price ({quote})</span><span>Amount ({base})</span></div>{trades.map(r=><div className="hs-trade-row" key={r.id??`${r.time}-${r.side}-${r.price}-${r.quantity}`}><span>{time(r.time)}</span><span>{pair}</span><span className={r.side==='BUY'?'bid':'ask'}>{r.side==='BUY'?'Buy':'Sell'}</span><span>{formatPriceValue(Number(r.price))}</span><span>{qty(Number(r.quantity))}</span></div>)}{!trades.length&&<span className="hs-empty">Data unavailable</span>}</div>
    <div className="ts-footer"><span>{stale?'Market data is delayed':'Market data'}</span></div>
  </div>;
}
