...
function PreviewCandles({ candles, label }: { candles: HomeCandle[]; label: string }) {
  ...
  return (
    <svg viewBox="0 0 600 200" preserveAspectRatio="none" role="img" aria-label={label} className="vx-real-candles">
      ...
      <line x1="0" x2="548" y1={y(last.close)} y2={y(last.close)}
        stroke={last.close >= last.open ? '#2ebd85' : '#f0616d'} strokeWidth=".6" strokeDasharray="3 4" />
    </svg>
  );
}
    ...
    <div id="home-live-terminal" className="vx-live-terminal" data-stale={!!stale}>
      ...
      <div className="vx-terminal-instrument">
        <div className="vx-terminal-pair">
          ...
          <strong>{pair}</strong>
          <LiveValue value={lead?.price} className={up ? 'text-up' : 'text-down'} />
        </div>
      ...
      <div className="vx-terminal-workspace">
        ...
        <section className="vx-terminal-center">
          ...
          <div className="vx-terminal-chart">
            {candles.length > 0
              ? <PreviewCandles candles={candles} label={`${pair} · ${copy.candles}`} />
        ...
        <aside className="vx-terminal-book">
          ...
          <div className="vx-terminal-book-note">
            {feed?.bookStatus === 'error' && book ? t('analytics.stale') : copy.refresh}
          </div>
