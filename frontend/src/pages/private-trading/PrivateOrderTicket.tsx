...
export function PrivateOrderTicket({symbol,market,wallet,busy,preview,pickedPrice,savedPreviews=[],onResume,onRefresh,onModeChange,onPreview,onConfirm,onCancel,onAllocate}:{
  ...
  return <aside className="private-order-ticket">
    ...
    <div className="private-wallet"><span>Доступно</span><strong>{privateNumber(wallet?.available)} USDT</strong><details><summary>Выделить демо-капитал</summary><p>Демо-депозит: {privateNumber(wallet?.demoAvailable)} USDT</p><label>Сумма, USDT<input inputMode="decimal" type="number" min="0.00000001" step="any" value={allocation} onChange={e=>setAllocation(e.target.value)}/></label><button type="button" disabled={busy||!allocation||Number(allocation)<=0} onClick={()=>onAllocate(allocation)}>Выделить</button></details></div>
    ...
    {preview&&<section className="private-preview" aria-live="polite"><header><strong>Предпросмотр · {livePreview?'Сейчас':'Исторический тест'}</strong><button type="button" disabled={busy} onClick={onCancel} aria-label="Закрыть предпросмотр">×</button></header>
