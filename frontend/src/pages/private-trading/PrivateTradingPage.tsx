...
function PrivateTradingWorkspace({onDenied}:{onDenied:()=>void}){
  ...
  return <>
    ...
    <main className="private-terminal-grid">
      ...
      <div className="private-book repaired-futures-book"><FuturesReferenceBook key={symbol} pair={symbol} bids={market?.bids??[]} asks={market?.asks??[]} lastPrice={market?Number(market.lastPrice):null} markPrice={market&&market.markPrice!=null?Number(market.markPrice):null} onPickPrice={price=>setPickedPrice(current=>({price,sequence:(current?.sequence??0)+1}))}/></div>
