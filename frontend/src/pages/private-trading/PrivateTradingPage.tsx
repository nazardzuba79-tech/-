  ...
  return <>
    ...
    <main className="private-terminal-grid">
      ...
      <PrivateOrderTicket key={symbol} symbol={symbol} market={market} wallet={state?.wallet??null} busy={busy} preview={preview} pickedPrice={pickedPrice} savedPreviews={state?.previews??[]}
        ...
        onResume={id=>void run(`resume:${id}`,async()=>{const next=await privateTradingApi.getPreview(id);if(alive.current){if(next.result?.position)setSymbol(next.result.position.symbol.replace(/USDT$/,'/USDT'));setPreview(next);}})}
      ...
      {state?<PrivatePositions state={state} busy={busy} onAction={next=>{setError('');setAction(next);}}
