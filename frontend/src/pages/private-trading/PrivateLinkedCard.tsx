...
export function PrivateLinkedCard({id,onDenied}:{id:string;onDenied:()=>void}){
  ...
  return <main className="private-access-state"><h1>Карточка результата VOLTEX</h1>
    ...
    <Link to="/futures">Вернуться в приватный терминал</Link>
    {snapshot&&<PrivateResultCardDialog snapshot={snapshot} onClose={()=>navigate('/futures',{replace:true})} onError={cause=>{
