...
describe('native demo service (fixture market, in-memory persistence)',()=>{
  ...
  test('live market order: server sizes 5,000 x 20, stores only the consumed depth, refresh is cheap and not persisted',async()=>{
    ...
    const publishLive=jest.fn(async(_actor:OwnerSession,_revision:number,_account:NativeAccount)=>true);(f.repo as any).publishLive=publishLive;
...
describe('the Cross collateral base is the whole wallet, priced or named',()=>{
  ...
  test('a source that answers a valuation with a mark and no book prices the holding; a mark outside the window does not',async()=>{
    ...
    expect((await f.service.collateral(actor,undefined,{reuse:true})).priced).toBe('100100');
