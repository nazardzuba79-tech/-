...
describe('canonical checkpoint: live outcomes are never recomputed away',()=>{
  const tpOpen=(historical=true)=>open('o1',T,{historical,protection:{takeProfit:'50500',triggerBy:'LAST'}});
  ...
  test('a TP triggered by a live quote is journaled and survives later replays',()=>{
    const i=tpOpen(false),history=flat(T,T+4*M,'50000'),asOf=T+2*M+30_000;
    ...
    expect(live.snapshot.positions[0]).toMatchObject({status:'OPEN',pendingClose:{reason:'TAKE_PROFIT',quantity:'2'}});
    expect(live.snapshot.events.at(-1)).toMatchObject({kind:'TRIGGER',price:'50650',fee:'0',cashflow:'0',pricing:'LIVE_QUOTE_MODEL'});
    const observe:NativeInstruction={id:'observe-1',kind:'OBSERVE',at:asOf,seq:1,marks:live.observed!};
    const book:NativeInstruction={id:'book-1',kind:'BOOK',at:asOf,seq:2,symbol:'BTCUSDT',book:{timestamp:asOf,bids:[{price:'50640',quantity:'2'}],asks:[]}};
    const filled=replayNativeDemoWithBars({deposit:'1000000',instructions:[i,observe,book],bars:{BTCUSDT:history},asOf});
    ...
    expect(later.snapshot.events.at(-1)).toMatchObject({kind:'TAKE_PROFIT',price:'50640',pricing:'OBSERVED_BOOK'});
    expect(later.snapshot.walletBalance).toBe(filled.snapshot.walletBalance);
...
describe('cross account risk',()=>{
  ...
  test('an adverse move on one contract liquidates the whole cross account, not one isolated leg',()=>{
    const instructions:NativeInstruction[]=[open('a',T,{historical:true,quantity:'0.2'}),open('b',T,{historical:true,quantity:'4'},'2500','ETHUSDT')];
...
describe('orders, protection and funding through the replay',()=>{
  ...
  test('TP/SL can be added, changed and removed; the active SL triggers at its crossing',()=>{
    ...
    const s=replayNativeDemo({deposit:'1000000',instructions:[open('o1',T,{historical:true}),add,change,removeTp],bars,asOf:T+3*M});
