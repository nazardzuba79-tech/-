...
import { assertOwner, nativeAccessRow, privateTradingConfig, type PrivateTradingConfig } from '../access';
...
export async function assertNativeTrader(
  ...
  if (actor.userId === initial.ownerId || !isNativeTestAccount(actor.userId)) {
    return assertOwner(db, actor, config, now, true);
  }
  ...
  const access = await nativeAccessRow(db, actor.userId, actor.sessionId);
  if (!access || access.role !== 'USER' || access.blockedAt || access.revokedAt || access.sessionUserId !== actor.userId) throw denied();
  const final = config();
