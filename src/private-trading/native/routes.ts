...
export function nativeDemoRoutes(service:NativeDemoService,actor:(res:Response)=>OwnerSession){
  ...
  r.use((e:unknown,_req:Request,res:Response,next:NextFunction)=>{
    if(e instanceof DemoEngineError){
      // A code the trader has never seen is not a message. The code still
      // travels — the client maps it to its own localized text, and the
      // logs keep it — but the human-readable field never becomes one.
      if(!NATIVE_ERROR_TEXT[e.code])console.warn('[native] unmapped engine error',e.code);
