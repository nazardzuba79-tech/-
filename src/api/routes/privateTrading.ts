...
export const privatePreviewSchema = z.object({
  ...
  if (v.mode === 'HISTORICAL_REPLAY') {
    if (!v.effectiveOpenedAt) issue('Укажите время входа', ['effectiveOpenedAt']);
    if (v.type !== 'MARKET') issue('Для исторического сценария используется вход по следующей свече', ['type']);
  } else if (v.effectiveOpenedAt || v.effectiveClosedAt || v.asOf || v.manualEntryPrice || v.capital || v.events?.length) {
