...
], {providerWaitMs:1400,freshAgeMs:120_000});
...
async function bounded<T>(work: Promise<T>, fallback: T, maxWaitMs = 1400): Promise<T> {
  ...
  try { return await Promise.race([work.catch(() => fallback),new Promise<T>(resolve=>{timer=setTimeout(()=>resolve(fallback),maxWaitMs);})]); }
...
export function cfdRouter(prisma: PrismaClient, cfdDataService: CfdMarketDataService, positionService: CfdPositionService,
  displaySource: CfdDisplayQuoteRouter = defaultDisplaySource): Router {
  const router = Router();

  router.get('/admin/cfd/diagnostics', requireAuth(prisma), requireAdmin(prisma), async (_req, res) => {
    res.setHeader('Cache-Control', 'no-store');
    try { ensureDisplayFeeds();res.json({financial:await cfdDataService.diagnostics(),display:await displaySource.diagnostics()}); }
    catch { res.status(503).json({error:'cfd_diagnostics_unavailable'}); }
  });

  router.get('/cfd/config', (_req, res) => {
    res.json({symbols:CFD_SYMBOLS,catalog:cfdDataService.catalog(),maxQuoteAgeMs:cfdDataService.maxQuoteAgeMs,minLeverage:MIN_LEVERAGE,
      maxLeverage:MAX_LEVERAGE,newAccountMaxLeverage:NEW_ACCOUNT_MAX_LEVERAGE,newAccountPeriodDays:NEW_ACCOUNT_PERIOD_DAYS,
      highLeverageWarningThreshold:HIGH_LEVERAGE_WARNING_THRESHOLD,leverageTiers:LEVERAGE_TIERS});
  });
  ...
  router.get('/cfd/tickers', async (_req, res) => {
    try {
      ...
      const [primaryRows, displayRows] = await Promise.all([
        ...
        displaySource.getQuotes(),
      ]);
      const primary=new Map(primaryRows.map(q=>[q.symbol,q])), alternatives=new Map(displayRows.map(q=>[q.symbol,q]));
      ...
      const tickers=CFD_REFERENCE_CATALOG.map(base=>{
        ...
        const q=displaySource.choose(financial,alternatives.get(base.symbol));
        const status=displayStatus(q),asOf=q.providerTimestamp??q.fetchedAt;
        return {symbol:q.symbol,name:catalog.find(i=>i.symbol===q.symbol)?.name??base.name,
          price:q.last===null?null:q.lastDecimal??String(q.last),changePercent24h:q.changePercent24h,status,stale:status==='stale',
          marketClosed:status==='market_closed',displayOnly:true,executionAllowed:false,entitlementVerified:false,
          provider:q.provider,providerSymbol:q.providerSymbol,providerTimestamp:q.providerTimestamp,fetchedAt:q.fetchedAt,
          asOf:typeof asOf==='number'&&Number.isFinite(asOf)?asOf:null,maxQuoteAgeMs:displaySource.freshAgeMs};
      });
      res.setHeader('Cache-Control','no-store');
      res.json({source:'voltex-multi-source-display',configured:true,tickers});
    } catch (err) { console.error(err); res.status(500).json({error:'Internal server error'}); }
  });

  // Existing financial endpoints stay untouched and continue to use the
  // original CfdMarketDataService. They are not used by the read-only UI.
  router.post('/cfd/positions', requireAuthOrApiKey(prisma), requireTradePermission, async (req: ApiAuthedRequest, res) => {
    const parsed=openSchema.safeParse(req.body);if(!parsed.success)return res.status(400).json({error:parsed.error.issues[0]?.message??'Invalid request'});
    try {const position=await positionService.open({userId:req.userId!,symbol:parsed.data.symbol,side:parsed.data.side,quantity:new BigNumber(parsed.data.quantity),leverage:parsed.data.leverage});res.json({position:serializePosition(position)});}
  ...
  router.get('/cfd/positions', requireAuthOrApiKey(prisma), async (req: ApiAuthedRequest, res) => {
    const positions=await positionService.listOpen(req.userId!);let tickers:{symbol:string;price:string}[]=[];
    try{const quotes=await cfdDataService.getQuotes();tickers=quotes.flatMap(q=>{try{return[{symbol:q.symbol,price:String(assertCfdFreshQuote(q,q.symbol,cfdDataService.maxQuoteAgeMs))}];}catch{return[];}});}catch{}
    const priceBySymbol=new Map(tickers.map(t=>[t.symbol,new BigNumber(t.price)]));
    res.json(positions.map(p=>{const markPrice=priceBySymbol.get(p.symbol)??null,size=new BigNumber(p.size.toString()),entryPrice=new BigNumber(p.entryPrice.toString()),initialMargin=new BigNumber(p.initialMargin.toString());const unrealizedPnl=markPrice?computeUnrealizedPnl(p.side as PositionSide,size,entryPrice,markPrice):null;const roe=unrealizedPnl?computeROE(unrealizedPnl,initialMargin):null;return{...serializePosition(p),markPrice:markPrice?.toString()??null,unrealizedPnl:unrealizedPnl?.toString()??null,roe:roe?roe.times(100).toString():null};}));
  });
