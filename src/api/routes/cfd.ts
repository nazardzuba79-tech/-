...
async function bounded<T>(work:Promise<T>,fallback:T,maxWaitMs=1400):Promise<T>{let timer:NodeJS.Timeout|undefined;try{return await Promise.race([work.catch(()=>fallback),new Promise<T>(resolve=>{timer=setTimeout(()=>resolve(fallback),maxWaitMs);})]);}finally{if(timer)clearTimeout(timer);}}
...
export function cfdRouter(prisma:PrismaClient,cfdDataService:CfdMarketDataService,positionService:CfdPositionService,
  ...
  // original CfdMarketDataService. The read-only UI never calls them.
