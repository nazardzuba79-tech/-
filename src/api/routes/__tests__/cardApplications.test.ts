...
describe('real card eligibility', () => {
  test.each([
    ...
    ['owner A', 'NOT_STARTED', '5000.01', '50000.01', false],
    ['owner B', 'APPROVED', '4999.99', '49999.99', false],
  ])('%s: KYC=%s deposit=%s volume=%s gives eligible=%s', async (_case, kyc, amount, volume, expected) => {
    const { service, prisma } = fixture({ kyc, deposits: [deposit(amount)], trades: volume === '0' ? [] : [trade(volume)] });
    ...
    if (expected) {
      ...
      await expect(service.submit(userId, 'TITANIUM')).rejects.toMatchObject({ statusCode: 403, code: 'CARD_NOT_ELIGIBLE' });
  ...
  it('owner E: a deposit above 5000 allows submission even when the volume route cannot be valued', async () => {
    const { service, prisma } = fixture({ deposits: [deposit('5000.01')], trades: [trade('100000', { pair: 'BTC/UNKNOWN' })] });
    const result = await service.submit(userId, 'BLACK_SIGNATURE');
    expect(result.eligibility).toMatchObject({
      verificationApproved: true, depositEligible: true, tradingVolumeEligible: false,
      eligible: true, qualifyingDepositUsd: 5000.01, depositValuationComplete: true,
      qualifyingTradingVolumeUsd: 0, tradingVolumeValuationComplete: false,
    });
    expect(result.application).toMatchObject({ product: 'BLACK_SIGNATURE', status: 'SUBMITTED' });
...
describe('persisted card applications', () => {
  ...
  it('rechecks credited deposits inside the transaction instead of trusting the earlier quote lookup inputs', async () => {
    const { service, prisma } = fixture();
    prisma.deposit.findMany.mockResolvedValueOnce([deposit('5000')]);
    await expect(service.submit(userId, 'TITANIUM')).rejects.toMatchObject({ code: 'CARD_NOT_ELIGIBLE' });
    expect(prisma.deposit.findMany).toHaveBeenCalledTimes(2);
    expect(prisma.cardApplication.create).not.toHaveBeenCalled();
    expect(prisma.auditLog.create).not.toHaveBeenCalled();
  });

  it('preserves an existing application and its original evidence after current eligibility is lost', async () => {
    const { service, prisma, state } = fixture({ deposits: [deposit('5000')] });
    const first = await service.submit(userId, 'TITANIUM');
    const storedEvidence = JSON.stringify(state.application);
    prisma.user.findUnique.mockResolvedValue({ id: userId, kycStatus: 'REJECTED', blockedAt: null });
    prisma.deposit.findMany.mockResolvedValue([]);
    const replay = await service.submit(userId, 'TITANIUM');
    expect(replay.application).toEqual(first.application);
    expect(replay.eligibility.eligible).toBe(false);
    expect(JSON.stringify(state.application)).toBe(storedEvidence);
    expect(prisma.cardApplication.create).toHaveBeenCalledTimes(1);
    expect(prisma.auditLog.create).toHaveBeenCalledTimes(1);
  });

  it('propagates audit failure through the same transaction so application creation rolls back', async () => {
    const { service, prisma, state } = fixture({ deposits: [deposit('5000')] });
    // Local transaction adapter models rollback; production atomicity is the
    // real Prisma serializable transaction, not compensating application writes.
    prisma.$transaction.mockImplementation(async (callback: (tx: any) => Promise<unknown>) => {
      ...
      catch (error) { state.application = previous; throw error; }
    });
    const unavailable = new Error('audit unavailable');
    prisma.auditLog.create.mockRejectedValue(unavailable);
    await expect(service.submit(userId, 'TITANIUM')).rejects.toBe(unavailable);
...
describe('card API authorization and validation', () => {
  ...
  it('isolates two authenticated accounts on both submission and subsequent GET', async () => {
    const { app, prisma } = fixture({ deposits: [deposit('5000')] });
    const records = new Map<string, any>();
    prisma.user.findUnique.mockImplementation(async ({ where }: any) => ({ id: where.id, kycStatus: 'APPROVED', blockedAt: null }));
    prisma.cardApplication.findUnique.mockImplementation(async ({ where }: any) => records.get(where.userId) ?? null);
    prisma.cardApplication.create.mockImplementation(async ({ data }: any) => {
      const application = { id: `application-${data.userId}`, submittedAt: new Date('2026-09-06T12:00:00Z'), ...data };
      records.set(data.userId, application);
      return application;
    });
    const otherAuth = `Bearer ${jwt.sign({ sub: 'user-2' }, process.env.JWT_SECRET!)}`;
    const first = await request(app).post('/api/v1/card/application').set('Authorization', authHeader()).send({ product: 'TITANIUM' });
    const second = await request(app).post('/api/v1/card/application').set('Authorization', otherAuth).send({ product: 'BLACK_SIGNATURE' });
    expect([first.status, second.status]).toEqual([200, 200]);
    const firstRead = await request(app).get('/api/v1/card/application/me').set('Authorization', authHeader());
    const secondRead = await request(app).get('/api/v1/card/application/me').set('Authorization', otherAuth);
    expect(firstRead.body.application).toEqual(first.body.application);
    expect(secondRead.body.application).toEqual(second.body.application);
    expect(firstRead.body.application.id).not.toBe(secondRead.body.application.id);
    expect(prisma.deposit.findMany).toHaveBeenCalledWith({ where: { userId: 'user-2', status: 'CREDITED' }, select: { asset: true, amount: true } });
    expect(records.size).toBe(2);
  });

  it('marks successful, denied, invalid and unauthenticated application responses no-store', async () => {
    const { app } = fixture({ deposits: [deposit('5000')] });
    const responses = [
      await request(app).get('/api/v1/card/application/me'),
      await request(app).post('/api/v1/card/application').send({ product: 'TITANIUM' }),
      await request(app).post('/api/v1/card/application').set('Authorization', authHeader()).send({ product: 'INVALID' }),
      await request(app).post('/api/v1/card/application').set('Authorization', authHeader()).send({ product: 'TITANIUM' }),
      await request(app).post('/api/v1/card/application').set('Authorization', authHeader()).send({ product: 'BLACK_SIGNATURE' }),
    ];
    expect(responses.map(response => response.status)).toEqual([401, 401, 400, 200, 409]);
    for (const response of responses) expect(response.headers['cache-control']).toBe('no-store');
    const ineligible = fixture();
    const denial = await request(ineligible.app).post('/api/v1/card/application').set('Authorization', authHeader()).send({ product: 'TITANIUM' });
