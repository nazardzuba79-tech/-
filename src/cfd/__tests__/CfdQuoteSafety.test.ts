...
import { assertCfdFreshQuote, assertCfdOpenQuote, quoteAgeLimit, type CfdQuote, type CfdQuoteSource } from '../../services/marketData/cfd/CfdQuote';
...
const quote = (over: Partial<CfdQuote>={}):CfdQuote => ({provider:'test',symbol:'XAUUSD',providerSymbol:'XAU/USD',
  bid:null,ask:null,mid:null,last:1800,providerTimestamp:at,fetchedAt:at,stale:false,status:'live',entitlementVerified:true,executionAllowed:true,...over});
const invalid: [string, Partial<CfdQuote>][] = [
  ...
  ['unverified live entitlement',{entitlementVerified:false}],['missing entitlement',{entitlementVerified:undefined}],
  ...
  ['malformed decimal',{lastDecimal:'not-a-price'}],['mismatched decimal',{lastDecimal:'100'}],
];
...
test('fresh single reference price accepted with null bid/ask/mid, no synthetic spread',()=>{
  const q=quote();expect(assertCfdOpenQuote(q,'XAUUSD',5000,at)).toBe(1800);expect(q).toMatchObject({bid:null,ask:null,mid:null});
});
test('preserves the exact provider decimal for existing BigNumber accounting',async()=>{
  ...
  const q=await service.getFreshQuote('XAUUSD');
  expect(new BigNumber(assertCfdOpenQuote(q,'XAUUSD',5000,at)).toString()).toBe('2000.123456789012345678');
...
test.each(invalidRisk)('open, close and liquidation make no financial writes on %s',async(_,over)=>{
...
test.each([null,undefined,0,(at-60000)/1000,(at+60000)/1000])('missing/old/future provider timestamp %p cannot execute',async(timestamp)=>{
  const {service}=adapter({'XAU/USD':{close:'2000',timestamp}});await expect(service.getFreshQuote('XAUUSD')).rejects.toThrow();
});
test('support, entitlement, actual freshness and execution approval remain independent',async()=>{
  ...
  expect((await service.getQuotes()).find(q=>q.symbol==='WTIUSD')).toMatchObject({status:'unavailable',last:null,bid:null,ask:null,entitlementVerified:false});
  await expect(service.getFreshQuote('XAUUSD')).rejects.toThrow();
  expect(()=>adapter({}, {entitledSymbols:['GUESS']})).toThrow('Unverified');
});
test('provider outage retains a flagged stale display but never an old execution price',async()=>{
  ...
  await expect(service.getFreshQuote('XAUUSD')).rejects.toThrow();
  expect((await service.getQuotes()).find(q=>q.symbol==='XAUUSD')).toMatchObject({status:'stale',stale:true,executionAllowed:true,last:2000});
});
