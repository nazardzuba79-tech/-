 ...
 * Never used by CFD position/accounting code. */
export class EiaOilDisplaySource{
  ...
  private parse(html:string,symbol:keyof typeof SERIES,receivedAt:number):CfdQuote{
    ...
    for(const row of html.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)){
      ...
      const month=MONTHS.indexOf(m[2]);if(month<0)continue;const start=Date.UTC(Number(m[1]),month,Number(m[3]));
      if(new Date(start).getUTCDay()!==1)continue;
  ...
  async getQuotes():Promise<CfdQuote[]>{
    ...
    const run=(async()=>{const out:CfdQuote[]=[];for(const symbol of ['WTIUSD','XBRUSD'] as const){const meta=SERIES[symbol];try{const r=await this.fetchFn(`https://www.eia.gov/dnav/pet/hist/${meta.file}`,{redirect:'error',signal:AbortSignal.timeout(this.timeoutMs),headers:{Accept:'text/html','User-Agent':'VOLTEX-market-display/1.0'}});if(!r.ok){await r.body?.cancel();out.push(this.missing(symbol,meta.providerSymbol));continue;}const html=await r.text();out.push(html.length<=2_000_000?this.parse(html,symbol,this.now()):this.missing(symbol,meta.providerSymbol));}catch{out.push(this.missing(symbol,meta.providerSymbol));}}
